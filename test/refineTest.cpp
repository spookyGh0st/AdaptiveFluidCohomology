...
#include "eider/AdaptiveHomologyBasis.h"
...
TEST(refineTest, uniform_vs_adaptive_refinement) {
    ...
    while (uniform_m.nFaces() < 3072) {
        uniform_refine(uniform_a);
        uniform_refine(adaptive_a);
    }
    ...
    auto adaptive_refine = [&]() {
        VertexData<double> f(adaptive_m, 1);
        VertexData<double> u(adaptive_m, 0);
        StreamFunctionSolver S{};
        S.compute(adaptive_m, adaptive_a.geom());
        S.solve(adaptive_m, adaptive_a.geom(), u, f);
    ...
    polyscope::state::userCallback = [&]() {
        ...
        if (ImGui::Button("Refine until 2xT")) {
            adaptive_refine();
            while (uniform_m.nFaces() * 2 < adaptive_m.nFaces())
                uniform_refine(uniform_a);
            vis_all();
        }
...
TEST(refineTest, testCollapsingSameElement) {
    ...
    i = 0;
    for (Edge e : he.vertex().adjacentEdges())
        b_edges[i++] = e;
...
TEST(refine, testPathConsistency) {
    std::filesystem::path fds(__FILE__);
    ...
    for (int h_idx = 0; h_idx < h.size(); ++h_idx) {
        icit.edgeSplitCallbackList.emplace_back([&, h_idx](Edge e, Halfedge he1, Halfedge he2) { onSplit(e, he1, he2, h[h_idx].nextLeft); });
    }
    for (int h_idx = 0; h_idx < h.size(); ++h_idx) {
        icit.edgeCollapseCallbackList.push_back([&, h_idx](Halfedge he) { onCollapse(he, h[h_idx].nextLeft); });
    }
