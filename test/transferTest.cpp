...
TEST(transferTest, testAdaptiveTangentSpaceSplitHe) {
    ...
    for (Halfedge h : f.adjacentHalfedges())
        atri.marked_corner[h.corner()] = false;
...
TEST(transferTest, testSplitPreserveFace) {
    ...
    for (Edge e : m.edges())
        if (!e.isBoundary())
            he = e.halfedge(); // should also hold for twin
    for (Face f : he.edge().adjacentFaces())
        fd[f] = true;
