...
void copyFolder(const fs::path &source, const fs::path &destination) {
    try {
        ...
        if (fs::exists(destination)) {
            if (!destination.filename().string().starts_with("run_tc")) {
                throw std::runtime_error("Careful, we delete it here.");
            }
...
double smallest_edge_length(IntrinsicGeometryInterface &geom) {
    ...
    for (Edge e : geom.mesh.edges()) {
        if (geom.edgeLengths[e] < min)
            min = geom.edgeLengths[e];
    }
    return min;
}
  ...
  public:
    ...
    polyscope::SurfaceMesh *visualize(fs::path f_screenshots) override {
        ...
        if (!f_screenshots.empty()) {
            ...
            for (Face f : m.faces()) {
                e1[f] = g.faceTangentBasis[f][0], e2[f] = g.faceTangentBasis[f][1];
            }
...
std::shared_ptr<TaylorVorticesCase> makeTaylorCase(
    std::string name,
    std::string label,
    ManifoldSurfaceMesh &pmesh,
    VertexPositionGeometry &pgeom,
    ...
    if (d.adaptive_space) {
        refineAndReset(*aCase, aCase->geom, taylor);
    }
...
TEST(EvaluatorTest, EvaluateDiffDopri) {
    ...
    cpm.runUntil(1. / 2 * target_t);
    cpm.visualize(cf.f_screenshots);
    cpm.runUntil(2. / 2 * target_t);
    cpm.visualize(cf.f_screenshots);
...
TEST(EvaluatorTest, EvaluateAdapt) {
    ...
    cpm.testcases = {tc2, tc1};
    cmp2.testcases = {tc2, tc3};
    init_ps(cpm);
    init_ps(cmp2);
...
TEST(EvaluatorTest, evaluateBadTriangulation) {
    ...
    adaptDC.dt = 0.02;
    adaptDC.strategy = MARKING_STRATEGY::LONGEST_EDGE;
    adaptDC.adaptive_space = true;
    adaptDC.adaptive_time = true;
    adaptDC.doerflerConf = DoerflerPreset(DoerflerPresetConf::LOW);
    AdaptiveFluidSolverData adaptDI = adaptDC;
    adaptDI.interpolate_harmonic_basis = true;
    adaptDI.use_interpolated_harmonic_basis = true;
    ...
    cpm.runUntil(1. / 4 * target_t);
    cpm.visualize(cf.f_screenshots);
    cpm.runUntil(2. / 4 * target_t);
    cpm.visualize(cf.f_screenshots);
    cpm.runUntil(3. / 4 * target_t);
    cpm.visualize(cf.f_screenshots);
    cpm.runUntil(4. / 4 * target_t);
    cpm.visualize(cf.f_screenshots);
...
TEST(EvaluatorTest, evaluateAdaptiveResidual) {
    ...
    Evaluator uniform_ev{};
    Evaluator adaptive_ev{};
    Comparator cmp;
