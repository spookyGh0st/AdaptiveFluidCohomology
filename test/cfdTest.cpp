...
wc_wrapper init_taylor(SurfaceMesh &mesh, VertexPositionGeometry &geo, std::vector<FaceData<Vector2>> h, double vorticity_distance, Vector2 offset, Vector2 cuttof, Vector2 cuttof_offset) {
    ...
    for (Vertex v : mesh.vertices()) {
        ...
        if (abs(p.x + cuttof_offset.x) > cuttof.x || abs(p.y + cuttof_offset.y) > cuttof.y) {
            continue;
        }
...
TEST(cfdTest, IntrinsicHole) {
    ...
    std::vector<float> t_data({0});
    std::vector<std::vector<float>> c_data(h.size()), c_should(h.size());
    ...
    polyscope::state::userCallback = [&]() {
        ...
        ImGui::InputFloat("x_add", &x_add, 0.125, 0.5);
        ImGui::InputFloat("y_add", &y_add, 0.125, 0.5);
        ImGui::InputFloat("x_cuttof", &x_cuttof, 0.125, 0.5);
        ImGui::InputFloat("y_cuttoff", &y_cuttof, 0.125, 0.5);
        ImGui::InputFloat("x_cut offset", &x_cutt_offset, 0.125, 0.5);
        ImGui::InputFloat("y_cut offset", &y_cutt_offset, 0.125, 0.5);
        if (ImGui::Button("reset")) {
            ...
            for (int j = 0; j < c_data.size(); ++j) {
                ...
                c_should[j].clear();
                c_should[j].push_back(float(wc.c[j]));
            }
            vispm();
        };
