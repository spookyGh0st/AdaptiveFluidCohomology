...
void visualize_c_h0(AdaptiveFluidSolver &solver, VertexPositionGeometry &geom) {
    ...
    for (Face f : m.faces()) {
        e1[f] = g_vis.faceTangentBasis[f][0], e2[f] = g_vis.faceTangentBasis[f][1];
    }
...
TEST(VideoTest, Grid) {
    ...
    d.adaptive_space = false;
    d.adaptive_time = false;
    d.dt = 0.01;
...
TEST(VideoTest, adaptiveFC) {
    ...
    AdaptiveFluidSolverData d;
    d.adaptive_space = true;
    d.adaptive_time = true;
    d.dt = 0.01;
    AdaptiveFluidSolver solver(*mesh, *geom, d);
    solver.wc.w = TaylorInitializer().wc(solver.tri.intrinsicTriangulation(), *geom).w;
    for (int i = 0; i < 8; ++i) {
...
double double_shear_layer(double x, double y) {
    ...
    return delta * std::cos(2.0 * M_PI * x) + rho * sech2(rho * (y - 0.25)) - rho * sech2(rho * (y - 0.75));
}
