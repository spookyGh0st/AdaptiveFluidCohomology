...
TEST(homologyTest, TestHomotopyBasis) {
    ...
    FaceData<int> prev(*m, 0);
    for (Face f : m->faces()) {
        Halfedge he = dijkstra.first[f];
        if (he != Halfedge()) {
            prev[f] = he.face().getIndex();
        } else {
            prev[f] = -1;
        }
    }
    ...
    for (const auto &basis : co_loops) {
        ...
        for (Halfedge e : basis) {
            b[e.edge()] = 1;
        }
        for (Halfedge e : red_basis) {
            b_reduced[e.edge()] = 1;
        }
...
TEST(homologyTest, Intrinsic) {
    ...
    for (Vertex v : m->vertices()) {
        vd[im.vertex(v.getIndex())] = g->vertexPositions[v];
    }
    ...
    for (Face f : im.faces()) {
        e1[f] = vpg.faceTangentBasis[f][0], e2[f] = vpg.faceTangentBasis[f][1];
    }
